-- group by,having by --  

create database four;
use four;

create table course(crid int not null primary key,crname varchar(30));
insert into course(crid,crname) values(1,"BE"),(2,"BCA"),(3,"BCOM"),(4,"BE"),(5,"BHMS");

create table city(cid int not null primary key,cname varchar(30));
insert into city(cid,cname) values(1,"SURAT"),(2,"VAPI"),(3,"RAJKOT"),(4,"BARODA"),(5,"AHMEDABAD");

create table person(id int not null primary key,name varchar(30),age int,city int ,course int ,foreign key(city) references city(cid),foreign key(course) references course(crid));
insert into person(id,name,age,city,course) values(1,"ashok",24,2,4),(2,"vishal",22,1,3),(3,"sahil",19,3,2),(4,"vinod",25,1,4),(5,"ajay",21,2,3);

select count(age),age from person group by age;
select c.cname,count(p.city) from person p inner join city c on p.city=c.cid group by city;
select c.cname,count(p.city) from person p inner join city c on p.city=c.cid group by city order by count(p.city);
select c.cname,count(p.city) from person p inner join city c on p.city=c.cid where p.age>20 group by city;
select c.cname as City,count(p.city) as Total from person p inner join city c on p.city=c.cid group by city;


select count(age),age from person group by age having age<23;
select count(age),age from person group by age having age>21 order by count(age);



